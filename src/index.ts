...
const BASE = 'https://internal.open.piercecountywa.gov';
const UA = 'pipeworx-mcp-data-piercecounty/1.0 (+https://pipeworx.io)';

const tools: McpToolExport['tools'] = [
  {
    name: 'datasets',
    description: 'Search the Pierce County Open Data catalog of open datasets by keyword. Returns each dataset\'s resource_id, name, description, category and update date — pass the resource_id to query/metadata.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'query',
    description: 'Run a Socrata SoQL query against a Pierce County Open Data dataset by resource_id (e.g. "hmbh-c3hw"). Filter with where/select/group/order (SoQL clauses, without the leading $) plus limit/offset. Returns matching rows as JSON.',
    inputSchema: {
      type: 'object',
      properties: {
        resource_id: { type: 'string', description: 'Dataset id, e.g. "hmbh-c3hw" (from datasets).' },
  ...
  {
    name: 'metadata',
    description: 'Get a Pierce County Open Data dataset\'s schema + metadata (columns, types, row count, category, last-updated) by resource_id, e.g. "hmbh-c3hw".',
    inputSchema: {
      type: 'object',
      properties: { resource_id: { type: 'string', description: 'Dataset id, e.g. "hmbh-c3hw".' } },
      required: ['resource_id'],
    },
  },
];
  ...
  switch (name) {
    case 'datasets': {
      const p = new URLSearchParams({
        ...
        domains: 'internal.open.piercecountywa.gov',
      });
    ...
    case 'query': {
      const id = reqStr(args, 'resource_id', '"hmbh-c3hw"');
      const p = new URLSearchParams();
    ...
    case 'metadata':
      return socrataGet(`/api/views/${reqStr(args, 'resource_id', '"hmbh-c3hw"')}.json`);
    default:
...
async function socrataGet(path: string): Promise<unknown> {
  ...
  if (!res.ok) throw new Error(`internal.open.piercecountywa.gov: ${res.status}`);
  return res.json();
}
